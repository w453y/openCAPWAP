...
static int memory_read(BIO *b, char *out, int outl)
{
	...
	CWLog("[BIO] memory_read waiting on list %p count=%d", pData->pRecvAddress, CWGetCountElementFromSafeList(pData->pRecvAddress));
    if (CWGetCountElementFromSafeList(pData->pRecvAddress) == 0) {
        struct timespec ts;
        clock_gettime(CLOCK_REALTIME, &ts);
        ts.tv_nsec += 100000000; /* 100ms */
        if (ts.tv_nsec >= 1000000000) { ts.tv_sec++; ts.tv_nsec -= 1000000000; }
        CWSafeList _l = pData->pRecvAddress;
        CWPrivateSafeList* _pl = (CWPrivateSafeList*)_l;
        pthread_cond_timedwait(_pl->pThreadCond, _pl->pThreadMutex, &ts);
        if (CWGetCountElementFromSafeList(pData->pRecvAddress) == 0) {
	...
	CWLog("[BIO] memory_read got %d bytes from list %p", size, pData->pRecvAddress);
    CWUnlockSafeList(pData->pRecvAddress);
...
static int memory_write(BIO *b, const char *in, int inl)
{
    ...
    errno = 0;
    /* Use send() for connected sockets (WTP), sendto() for unconnected (AC) */
    ret = send(pData->sock, strBuffer, inl + 4, 0);
    if (ret < 0 && (errno == EISCONN || errno == ENOTCONN)) {
...
static long memory_ctrl(BIO *b, int cmd, long num, void *ptr)
{
    ...
    switch (cmd) {
        ...
        case BIO_CTRL_PENDING:
            ret = CWGetCountElementFromSafeList(pData->pRecvAddress);
            break;
